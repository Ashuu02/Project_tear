...
const QUESTIONS = [
  {
    label: "Which dimensions matter most to you?",
    sub: "Select all that apply — we'll focus the research agents on these areas.",
  },
  {
    label: "What's your primary research goal?",
    sub: "This helps us frame the teardown from the right angle.",
  },
  {
    label: "How deep should the research go?",
    sub: "Choose the level of detail you need.",
  },
];

export default function IntakePage() {
  const router = useRouter();
  const { sessionId, productName, setTier1Answers } = useSessionStore();
    ...
    <div className="min-h-screen bg-tear-bg flex flex-col font-dm-sans text-tear-text">
      ...
      <div className="flex-1 overflow-y-auto py-10 px-6 pb-20">
        <div className="max-w-[720px] mx-auto flex flex-col gap-8">
          ...
          {step === 1 && (
            <DimensionGrid selected={dimensions} onToggle={toggleDimension} />
          )}
          {step === 2 && (
            <OptionGrid options={GOALS} selected={goal} onSelect={setGoal} />
          )}
          {step === 3 && (
            <OptionGrid options={DEPTHS} selected={depth} onSelect={setDepth} />
          )}
